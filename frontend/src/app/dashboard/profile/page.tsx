...
export default function ProfilePage() {
  ...
  // Active Tab state ("profile" | "bank" | "password")
  const [activeTab, setActiveTab] = useState<"profile" | "bank" | "password">("profile");

  // Sync profile details and listen to hash change
  useEffect(() => {
    syncProfile();

    const handleHash = () => {
      const hash = typeof window !== "undefined" ? window.location.hash : "";
      if (hash === "#bank-details" || hash === "#bank") {
        setActiveTab("bank");
      } else if (hash === "#change-password" || hash === "#password") {
  ...
  const changeTab = (tab: "profile" | "bank" | "password", hash: string) => {
    setActiveTab(tab);
  ...
  return (
    <div className="space-y-8 font-sans text-zinc-300 relative max-w-4xl">
      <div>
      ...
      <div className="flex flex-wrap items-center gap-2 p-1.5 bg-zinc-950 border border-zinc-900 rounded-2xl w-fit">
        <button
          onClick={() => changeTab("profile", "#profile")}
        ...
        <button
          onClick={() => changeTab("bank", "#bank-details")}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
            activeTab === "bank"
              ? "bg-[#ef233c] text-white shadow-lg shadow-[#ef233c]/30"
              : "text-zinc-400 hover:text-white hover:bg-zinc-900"
        ...
        <button
          onClick={() => changeTab("password", "#change-password")}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${
            activeTab === "password"
              ? "bg-[#ef233c] text-white shadow-lg shadow-[#ef233c]/30"
              : "text-zinc-400 hover:text-white hover:bg-zinc-900"
      ...
      {/* ── Section 2: Bank Account Details & UPI ──────────────────────── */}
      {activeTab === "bank" && (
        <div id="bank-details" className="bg-zinc-950 rounded-3xl p-6 sm:p-8 border border-zinc-900 shadow-xl text-white max-w-2xl">
      ...
      {activeTab === "password" && (
        <div id="change-password" className="bg-zinc-950 rounded-3xl p-6 sm:p-8 border border-zinc-900 shadow-xl text-white max-w-xl">
          <h3 className="font-extrabold text-lg text-white flex items-center gap-2 border-b border-zinc-900 pb-4 font-manrope">
